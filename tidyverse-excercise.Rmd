...
# install.packages('data.table', 'R.utils')
...
data_covid_sub %>%
  group_by(state, date) %>%
  summarize(sum(deaths), sum(cases), .groups = 'drop') %>%
  filter(date == max(date)) %>%
...
data_covid_sub %>%
  group_by(date) %>%
  summarize(tot_deaths = sum(deaths), tot_cases = sum(cases),  .groups = 'drop') %>%
  pivot_longer(cols = c(tot_deaths,tot_cases)) %>%
  ggplot(aes(x = date, y = value, colour = name)) + 
  geom_line() + 
  scale_y_log10("Count") + 
  scale_x_date("Date", breaks = "3 months", date_labels = '%b-%y') +
  theme_minimal() +
...
g <- data_covid_sub %>%
  ...
  ggplot(aes(x = date, y = cases, color = state_code)) + 
  geom_line() #+ 
  #scale_y_log10()
plotly::ggplotly(g)
...
g <- 
  data_covid_sub %>%
  filter(state_code %in% c("NY", "TX", "CA", "FL", "IL")) %>%
  group_by(state_code, date) %>%
  summarise(cases = sum(cases)) %>%
  mutate(d_cases = cases - lag(cases)) %>%
  ggplot(aes(x = date, y = d_cases, color = state_code)) + geom_line()
